...
<div class="single-media-section pt-3 pb-3">
  <div class="page-cover"
      style="background-image: url('{{ asset('uploads/'.$movies->image) }}')"></div>
          ...
          <div class="col-8 col-md-4 col-lg-3 m-auto m-sm-0">
              <div class="single-media-box mt-2">
                  <div class="media-thumb"
                      style="background-image: url('{{ asset('uploads/'.$movies->image) }}');">
                  </div>
              </div>
                      ...
                      <ul class="single-info-group">
                          ...
                          <li>
                            ...
                            <a 
                                ...
                                @endif
                                class="category">{{ DB::table("type")->where("id", $movies->type_id)->value("name_type") ?? "" }}
                            </a>
                            </li>
                      </ul>
...
<div class="section-watch pt-5 pb-3">
  <div class="container">
      <div class="row">
          <div class="col-md-10 media-player pe-md-0">
              <!-- start list-servers -->
          ...
          <div class="col-12 col-md-2 episodes-container">
              <!-- start seasons-dropdown -->
              <div class="dropdown seasons-dropdown">
                  <button type="button" class="btn btn-primary btn-season dropdown-toggle my-toggle"
                      data-bs-toggle="dropdown">
                      {{ $chapter_select->name_chapter }}
                  </button>
                  <ul class="dropdown-menu">
                    @foreach($chapter as $item)
                      <li><a class="dropdown-item" href="{{ request()->path }}?session={{ $item->slug }}">{{ $item->name_chapter }}</a></li>
                    @endforeach
                  </ul>
              </div>

              <ul class="episodes-list nice-scroll render-sesion">
                @foreach($lesson as $item)
                    @php
                        $chapter = DB::table("chapter")->where("id", $item->chapter_id)->value("slug") ?? "";
                    @endphp
                  <li class="episode"><a href="{{ request()->path }}?session={{ $chapter }}&lesson={{ $item->id }}" style="color: {{ (\request()->lesson == $item->id) ? 'blue' : "" }}">{{ $item->name_lesson }}</a></li>
                @endforeach
              </ul>
              <!-- </div> -->
          </div>
...
<section class="servers-download-list pt-4 pb-4">
    <div class="container">
              ...
              <div class="media-box">
                ...
                @endif
                  ...
                  <div class="rate-quality-info">
                      ...
                      <span class="rate icofont-star">8.5</span>
                  </div>
