	...
	<div class="card-body">
		<table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example">
          ...
          <th class="min-w-100px"><span style="color:rgb(68, 235, 62);">ID</span></th>
					...
					<th class="min-w-100px"><span style="color:rgb(68, 235, 62);">Content</span></th>
					<th class="min-w-100px"><span style="color:rgb(68, 235, 62);">Director</span></th>
        ...
        @foreach($blog as $item)
          <tr class="odd">
            ...
            <td class="min-w-100px">{{ $item->content }}</td>
            <td cclass="min-w-100px">{{ $item->director }}</td>
            <td class="min-w-100px">{!! Str::limit($item->intro, 100) !!}</td>          
            <td class="min-w-75px"><img width="50px" src="{{ asset('uploads/'. $item->image) }}" /></td>
            @if ($item->status == 1)
              <td class="min-w-100px">Not stand out</td>
            @elseif ($item->status == 2)
            <td class="min-w-100px">Outstanding</td>
            @else
              <td class="min-w-100px">Does not exist</td>
            @endif
            <td class="min-w-100px"><a href="{{ route('admin.blog.delete', ['id'=>$item->id]) }}">Delete</a></td>
            <td class="min-w-100px"><a href="{{ route('admin.blog.edit', ['id'=>$item->id]) }}">Edit</a></td>
          </tr>
        @endforeach
			</tbody>
		</table>
