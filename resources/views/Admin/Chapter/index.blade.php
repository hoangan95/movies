	...
	<div class="card-body">
		<table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example">
          ...
          <th class="min-w-100px"><span style="color:rgb(68, 235, 62);">ID</span></th>
					<th class="min-w-100px"><span style="color:rgb(68, 235, 62);">Name Chapter</span></th>
					<th class="min-w-100px"><span style="color:rgb(68, 235, 62);">Name Movies</span></th>
        ...
        @foreach($chapter as $item)
          <tr class="odd">
            ...
            <td class="min-w-100px">{{ $item->name_chapter }}</td>
            <td class="min-w-100px">{{ DB::table("movies")->where("id", $item->movie_id)->value("name") ?? "" }}</td>
            <td class="min-w-100px"><a href="{{ route('admin.chapter.delete', ['id'=>$item->id]) }}">Delete</a></td>
            <td class="min-w-100px"><a href="{{ route('admin.chapter.edit', ['id'=>$item->id]) }}">Edit</a></td>
          </tr>
        @endforeach
			</tbody>
		</table>
