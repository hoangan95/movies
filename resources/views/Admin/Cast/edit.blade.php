...
@section('content')
<form action="{{ route('admin.cast.update',['id'=>$cast->id]) }}" method="post" enctype="multipart/form-data">
  @csrf
            ...
            <div class="card">
              ...
              <div class="card-body">
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control form-control-solid" placeholder="Please create cast" name="name" value="{{ old('name',$cast->name)}}"/>
                </div>

                <div class="mb-3">
                  <label class="form-label">Image</label>
                  <input type="file" class="form-control" name='image' placeholder="Please create image">
                  <img width="50px" src="{{ asset('uploads/'.$cast->image) }}">
                </div>

                <div class="mb-3">
                  <label class="form-label">Intro</label>
                  <textarea  name='intro' type="text" class="form-control">{{ old('intro',$cast->intro)}}</textarea><script>
                            ...
                            <select class="form-select form-select-solid" data-control="select2" data-close-on-select="false" data-placeholder="Select an option" data-allow-clear="true" multiple="multiple"name="movie_id[]">
                                <option></option>
                                 @foreach($movies as $item)
                                   <option value="{{$item->id }}" {{ in_array( $item->id,$movie_id)? "selected" : '' }}>{{ $item->name }}</option>
                                  @endforeach
                            </select>
                        </div>
